...
void init_waterfall(){
	...
	if (wf)
	{
		printf("Freeing Waterfall\n");		// N3SB Hack
		free(wf);
	}
	//this will store the db values of waterfall
	printf("MAX_BINS/2: %d, f->height: %d\n", MAX_BINS/2, f->height);		// N3SB Hack	
	if (f->height > 0)
	{
...
void field_move(char *field_label, int x, int y, int width, int height){
	...
	if(height < 0)
	{
		printf("field_move given height less than 0\n");			// N3SB Hack
		printf("field_label: %s\n",field_label);				// N3SB Hack
	}
	
	update_field(f);
	if (!strcmp(field_label, "WATERFALL"))
	{
		printf("Calling init_waterfall\n");
		init_waterfall();
	}
}
...
static void layout_ui(){
	...
	switch(m_id){
		...
		case MODE_CWR:
			...
			printf("y2: %d, y1: %d\n", y2, y1);				// N3SB Hack
			printf("Calling field_move with height: %d\n", y2-y1-175);				// N3SB Hack
